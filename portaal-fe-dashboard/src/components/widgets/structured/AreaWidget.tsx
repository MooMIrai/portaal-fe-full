...
import { AreaConfig } from '../../../types/widgetConfig.types';

// Register Chart.js components (including Filler for area charts)
ChartJS.register(CategoryScale, LinearScale, PointElement, LineElement, Title, Tooltip, Legend, Filler);

export const AreaWidget: React.FC<WidgetProps<AreaConfig>> = ({ data, config }) => {
  ...
  const chartJSData: ChartData<'line'> = {
    ...
    datasets: valueFields.map((field, index) => ({
      ...
      backgroundColor: config.fillOpacity !== undefined 
        ? `${colors[index] || colors[0]}${Math.round(config.fillOpacity * 255).toString(16).padStart(2, '0')}`
        : (colors[index] || colors[0]).replace('rgb', 'rgba').replace(')', ', 0.3)'),
      fill: true,
