...
export class SafeXMLParser {
  ...
  private parseAttributeValue(value: string): any {
    // Handle dynamic placeholders: {$xxx$}
    const placeholderMatch = value.match(/^\{\$([^$]+)\$\}$/);
    if (placeholderMatch) {
      // Return placeholder with the exact name from template
      return { __placeholder: placeholderMatch[1] };
    }
    ...
    if (value.startsWith('{') && value.endsWith('}')) {
      try {
        ...
        // Handle dynamic placeholders: $xxx$
        const placeholderMatch = innerValue.match(/^\$([^$]+)\$$/);
        if (placeholderMatch) {
          return { __placeholder: placeholderMatch[1] };
        }
      } catch (e) {
