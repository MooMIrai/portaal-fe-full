...
// Services with special names
const services = [
  { dir: 'portaal-fe-lookUps', name: 'lookups' },
  { dir: 'portaal-fe-personalarea', name: 'personalarea' }
];

services.forEach(({ dir, name }) => {
  const webpackPath = path.join(__dirname, dir, 'webpack.config.js');
  
  if (!fs.existsSync(webpackPath)) {
    console.log(`❌ ${name}: webpack.config.js not found`);
    return;
  }
  ...
  if (content.includes('.module.')) {
    console.log(`✅ ${name}: Already has CSS modules`);
    return;
  }
              ...
              options: {
                modules: {
                  localIdentName: mode === 'development' 
                    ? "[name]__[local]__${name}_[hash:base64:5]"
                    : "[hash:base64:8]",
  ...
  console.log(`✅ ${name}: CSS modules configuration added`);
});

console.log('\n✅ CSS modules configuration complete for remaining services');